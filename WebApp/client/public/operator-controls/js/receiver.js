...
export class Receiver {
  ...
  async setupConnection(useWebSocket, codecs) {
    ...
    // open channel for op controls.
    this.opConChannel = this.pc.createDataChannel(this.connectionId, "data");
    this.opConChannel.onopen = function (){
      Logger.log("Operator control channel connected.");
    };
    this.opConChannel.onerror = function (e){
      Logger.log("The error " + e.error.message + " occurred\n while handling data with proxy server.");
    };
    this.opConChannel.onclose = function (){
      Logger.log("Operator control channel disconnected");
    }
  }
